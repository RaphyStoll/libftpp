...
static webserv::NetworkConfig g_config; // Empty config for testing
...
void testSimpleGet()
{
	...
	std::cout << "[Parsing] test1..." << std::endl;
	http::RequestParser parser;
	std::cout << "[Parsing] test2..." << std::endl;
	...
	http::RequestParser::State state =
		parser.parse(request.c_str(), request.size(), g_config);
...
void testPostBodyInChunks()
{
	...
	// Send headers first
	http::RequestParser::State state =
		parser.parse(headers.c_str(), headers.size(), g_config);
	...
	parser.parse(chunk1.c_str(), chunk1.size(), g_config);
	state = parser.parse(chunk2.c_str(), chunk2.size(), g_config);
...
void testChunkedIncrementalParsing()
{
	...
	parser.parse(part1.c_str(), part1.size(), g_config);
	parser.parse(part2.c_str(), part2.size(), g_config);
	http::RequestParser::State state = parser.parse(part3.c_str(), part3.size(), g_config);
...
void testIncrementalParsing()
{
	...
	parser.parse(chunk1.c_str(), chunk1.size(), g_config);
	parser.parse(chunk2.c_str(), chunk2.size(), g_config);
	http::RequestParser::State state =
		parser.parse(chunk3.c_str(), chunk3.size(), g_config);
...
void testParserReset()
{
	...
	std::string request1 = "GET /first HTTP/1.1\r\n"
						   "Host: example.com\r\n"
						   "\r\n";
	parser.parse(request1.c_str(), request1.size(), g_config);
	...
	std::string request2 = "POST /second HTTP/1.1\r\n"
						   ...
						   "test";
	http::RequestParser::State state =
		parser.parse(request2.c_str(), request2.size(), g_config);
